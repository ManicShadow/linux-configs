...
| Feature | Details |
| :--- | :--- |
| **Idempotent** | Safe to re-run — already-installed packages are skipped; dotfiles are updated/overwritten by default. |
...
| **Powerlevel10k Config** | Your personal `~/.p10k.zsh` is embedded directly — prompt is ready on first login, no wizard needed |
...
| **WSL Extras** | VBS silent launcher, Terminator icon, Windows shared-folder alias, and a custom Terminator Grid Layout plugin |
...
| :--- | :---: | :---: | :--- |
| Overwrite dotfiles? | `Y` | 🧹   | Overwrites .bashrc, .zshrc, .vimrc, .gitconfig, etc. |
...
| **Terminal** | `terminator`, `powerline`, `powerline-doc`, `powerline-gitstatus`, `python3-powerline-gitstatus`, `fonts-powerline`, `xclip` |
| **WSL / Display** | `xvfb`, `dbus-x11`, `mesa-utils`, `mesa-vulkan-drivers`, `xfonts-cyrillic`, `xfonts-scalable`, `libsecret-1-0` |
| **Fonts (apt)** | `fonts-font-awesome`, `fonts-noto-color-emoji`, `fonts-liberation`, `fonts-wqy-zenhei`, and others |
...
| `~/.vimrc` | vim-plug, NERDTree, Lightline, Nord colorscheme, CoC LSP, fzf.vim, GitGutter, auto-pairs |
| `~/.gitconfig` | `user.name/email`, GPG signing, `push.autoSetupRemote`, `pull.rebase`, LFS filters, `init.defaultBranch = main`, `core.filemode = false`, `[includeIf]` identity blocks, `cleanup` alias |
...
| `~/.p10k.zsh` | Your saved Powerlevel10k configuration — embedded directly, active on first login |
...
| Feature | Details |
| :--- | :--- |
| **VBS Launcher** | Writes `~/Documents/terminator-silent.vbs` — opens Terminator without a visible cmd window |
| **Terminator Icon** | Converts the installed `terminator.png` to `terminator.ico` (via ImageMagick) and saves it to `~/Documents/` |
...
2. **Target:** `C:\Windows\System32\wscript.exe "%USERPROFILE%\Documents\terminator-silent.vbs"`
