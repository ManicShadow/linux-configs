...
if [[ -f ~/.bashrc ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    ...
    if [[ -f ~/.bashrc ]]; then
        ...
        sudo cp ~/.bashrc "$BASHRC_BACKUP" 2>/dev/null || true
...
APT_PACKAGES=(
    ...
    zsh powerline powerline-doc powerline-gitstatus python3-powerline-gitstatus fonts-powerline terminator imagemagick
...
if [[ -d "$HOME/.oh-my-zsh" ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    ...
    if [[ -d "$HOME/.oh-my-zsh" ]]; then
        ...
        sudo mv "$HOME/.oh-my-zsh" "$HOME/.oh-my-zsh.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || sudo rm -rf "$HOME/.oh-my-zsh"
    fi
...
if [[ -f ~/.gitconfig ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    ...
    if [[ -n "$GIT_NAME" && -n "$GIT_EMAIL" ]]; then
        if [[ -f ~/.gitconfig ]]; then
            sudo cp ~/.gitconfig "$HOME/.gitconfig.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
...
if [[ "$GENERATE_SSH" == "y" && -n "$GIT_EMAIL" ]]; then
    if [[ -f ~/.ssh/id_ed25519 ]] && [[ "$OVERWRITE_KEYS" != "y" ]]; then
        ...
        if [[ -f ~/.ssh/id_ed25519 ]]; then
            sudo cp ~/.ssh/id_ed25519 "$HOME/.ssh/id_ed25519.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
            sudo cp ~/.ssh/id_ed25519.pub "$HOME/.ssh/id_ed25519.pub.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
...
if [[ -f ~/.zshrc ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    ...
    if [[ -f ~/.zshrc ]]; then
        sudo cp ~/.zshrc "$HOME/.zshrc.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
    fi
    sudo rm -f ~/.zshrc
...
function gacp {
    ...
    echo -e "\033[1;38;2;235;203;139m1. Staging all files (git add -A)...\033[0m"
    git add -A
    echo -e "\n\033[1;38;2;180;142;173m2. Committing...\033[0m"
    if ! git commit __COMMIT_FLAGS__ "$*"; then
        echo -e "\n\033[1;38;2;191;97;106mCommit failed! Aborting push.\033[0m"
        return 1
    fi
    echo -e "\n\033[1;38;2;136;192;208m3. Pushing to remote...\033[0m"
    if ! git push; then
        echo -e "\n\033[1;38;2;191;97;106mPush failed!\033[0m"
        return 1
    fi
    echo -e "\n\033[1;38;2;163;190;140mDone! Final status:\033[0m"
    git status
}
...
# ==============================================================================
# 🎨  P10K. POWERLEVEL10K CONFIG (~/.p10k.zsh)
...
# ==============================================================================
section "🎨  P10K. Powerlevel10k Config"

if [[ -f ~/.p10k.zsh ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    skip "~/.p10k.zsh already exists."
else
    if [[ -f ~/.p10k.zsh ]]; then
        # Use sudo cp in case the file is root-owned from a previous run
        sudo cp ~/.p10k.zsh "$HOME/.p10k.zsh.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
...
# ==============================================================================
# 📝  13. VIM-PLUG + VIM SETUP
# ==============================================================================
if [[ "$SETUP_VIM" == "y" ]]; then
    section "📝  13. Vim Config"
...
VIMRC_EOF
    ...
    section "📝  13. Vim Config — SKIPPED (user opt-out)"
fi


# ==============================================================================
# 📋  14. TMUX CONFIGURATION (Nord Theme)
# ==============================================================================
section "📋  14. Tmux Configuration"

if [[ -f ~/.tmux.conf ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    skip "~/.tmux.conf already exists."
else
    if [[ -f ~/.tmux.conf ]]; then
        sudo cp ~/.tmux.conf "$HOME/.tmux.conf.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
...
# ==============================================================================
# 🪟  15. TERMINATOR CONFIGURATION & PYTHON PLUGIN
# ==============================================================================
section "🪟  15. Terminator Configuration & Plugins"

if [[ -f ~/.config/terminator/config ]] && [[ "$OVERWRITE_DOTFILES" != "y" ]]; then
    skip "~/.config/terminator/config already exists."
else
    if [[ -f ~/.config/terminator/config ]]; then
        sudo cp ~/.config/terminator/config "$HOME/.config/terminator/config.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
...
[global_config]
  ...
  enabled_plugins = LaunchpadBugURLHandler, LaunchpadCodeURLHandler, APTURLHandler, TerminalShot, TerminatorThemes, GridTabIconizer
...
# ==============================================================================
# 🖥️   16. WINDOWS INTEGRATION (TERMINATOR GUI)
# ==============================================================================
section "🖥️  16. Windows Integration (Terminator)"
...
if $IS_WSL; then
    if [[ -n "$WINDOWS_USER" ]]; then
        ...
        if [[ -d "$WIN_DOCS" ]]; then
            info "Writing terminator-silent.vbs to Windows Documents..."
            # Note: The dbus-launch terminator -m command forces the GTK application
            # to boot completely maximized, effectively overriding any WSLg drawing quirks.
            cat << 'VBS_EOF' > "$WIN_DOCS/terminator-silent.vbs"
Set WshShell = CreateObject("WScript.Shell")
WshShell.Run "wsl.exe ~ -- sh -c ""mkdir -p /tmp/term-runtime && chmod 0700 /tmp/term-runtime && export XDG_RUNTIME_DIR=/tmp/term-runtime && export XCURSOR_SIZE=24 && terminator -u -m""", 0
VBS_EOF
            ...
            if command -v convert >/dev/null 2>&1; then
                ...
                if [[ -n "$ICON_SRC" ]]; then
                    convert -background none "$ICON_SRC" "$WIN_DOCS/terminator.ico" 2>/dev/null || true
                    success "VBS launcher and ICO file created in Windows Documents. 🪄"
                else
        ...
        skip "Windows username not provided, skipping Terminator integration."
    fi
else
    skip "Native Linux detected, skipping Windows specific integration."
fi


# ==============================================================================
# 👑  17. DEFAULT SHELL → ZSH
# ==============================================================================
section "👑  17. Setting Default Shell to ${DEFAULT_SHELL_CHOICE^^}"
...
# ==============================================================================
# 🎉  18. DONE — POST-INSTALL CHECKLIST
# ==============================================================================
section "🎉  18. Setup Complete!"
...
if $IS_WSL; then
    echo -e "${YELLOW}🪟  Terminator GUI (Windows Desktop Shortcut)${RESET}"
    if [[ -n "$WINDOWS_USER" && -f "/mnt/c/Users/${WINDOWS_USER}/Documents/terminator-silent.vbs" ]]; then
        echo -e "  ┣ Right-click your Windows Desktop -> New -> Shortcut"
        # Using printf with %s ensures literal string printing, making it immune to bash backslash parsing bugs
        printf "  ┣ Set Target: %b%s%b\n" "${BOLD}" 'C:\Windows\System32\wscript.exe "%USERPROFILE%\Documents\terminator-silent.vbs"' "${RESET}"
        echo -e "  ┣ Click Next, name it 'Terminator', and click Finish"
        printf "  ┗ Right-click Shortcut -> Properties -> Change Icon -> Browse to %b%s%b\n\n" "${BOLD}" 'Documents\terminator.ico' "${RESET}"
    else
        echo -e "  ┣ (Skipped automated VBS/ICO generation because Windows username was not provided)"
        echo -e "  ┗ Check the script source for manual VBS setup instructions.\n"
    fi
